...
function ReceiptPane({ task, onReveal, onRefresh }: { task: TaskDetail; onReveal: () => void; onRefresh: () => void }) {
  const pendingCount = task.validation.filter((item) => item.status !== "resolved").length;
  const resolvedCount = task.validation.filter((item) => item.status === "resolved").length;
...
function FourPieceStage({ task, refresh, notify }: { task: TaskDetail; refresh: (task: TaskDetail) => void; notify: (m: string) => void }) {
  ...
  const copyCompletion = async (scope: string) => { const prompt = await api.generatePrompt(task.id, "four-piece", { goal: scope, thinking: "基于现有资料和分析结果补齐，不编造缺失事实。", verification: "无法确认的内容保留为待验证项。" }); await navigator.clipboard.writeText(prompt); notify("四件套补齐调度单已复制"); };
  return <div className="document-workspace"><aside><div className="four-piece-head"><div><h2>四件套</h2><small>可手工维护，也可让AI基于现有资料补齐</small></div><AppButton icon={<Robot size={15} />} onClick={() => copyCompletion("检查并补齐全部四件套")}>AI补齐全部</AppButton></div>{task.requiredDocs.map((doc) => <button className={selected?.path === doc.path ? "active" : ""} key={doc.path} onClick={() => choose(doc)}><FileMd size={18} /><span><strong>{doc.name}</strong><small>{doc.filled ? "已填写" : "待补充"}</small></span><CaretRight size={14} /></button>)}</aside><section className="document-editor"><header><div><h2>{selected?.name}</h2><p>{selected?.path}</p></div><div className="inline-actions"><AppButton icon={<Robot size={15} />} onClick={() => selected && copyCompletion(`只检查并补齐 ${selected.name}`)}>AI补齐当前</AppButton><AppButton icon={<ArrowClockwise size={15} />} onClick={async () => refresh(await api.getTask(task.id))}>刷新</AppButton><AppButton tone="primary" onClick={async () => { if (selected) { await api.saveFile(selected.path, content); refresh(await api.getTask(task.id)); notify("文件已保存"); } }}>保存</AppButton></div></header><textarea value={content} onChange={(event) => setContent(event.target.value)} /></section></div>;
}
...
function ValidationStage({ task, refresh, notify }: { task: TaskDetail; refresh: (task: TaskDetail) => void; notify: (m: string) => void }) {
  ...
  return <div className="validation-workspace"><section className="validation-queue"><header><h2>验证队列</h2><span>待处理 {pending.length}</span></header>{pending.map((item) => <button className={selected?.id === item.id ? "active" : ""} key={item.id} onClick={() => setSelectedId(item.id)}><WarningCircle size={17} /><span><strong>{item.title}</strong><small>{item.source}</small></span><CaretRight size={15} /></button>)}{!pending.length ? <div className="queue-empty"><CheckCircle size={24} /><strong>暂无待确认问题</strong><span>仍可提交新的业务补充，再发起重分析。</span></div> : null}<button className={selectedId === "general" ? "active general-feedback" : "general-feedback"} onClick={() => setSelectedId("general")}><Plus size={17} /><span><strong>其他补充</strong><small>新增事实、口径或行动进展</small></span><CaretRight size={15} /></button></section><section className="validation-detail"><header><div><h2>{selected?.title || "补充新信息"}</h2><p>先提交补充，再开始重分析；两步不会互相阻塞。</p></div><AppButton icon={<Robot size={15} />} onClick={dispatch}>开始重分析</AppButton></header>{selected ? <><div className="detail-block"><span>问题说明</span><p>{selected.description}</p></div><div className="detail-block"><span>证据位置</span><p>{selected.source || "未记录"}</p></div></> : <div className="detail-block"><span>适用场景</span><p>补充首次分析未覆盖的新事实、指标口径、业务确认或行动进展。</p></div>}<label className="field-label">人工确认与补充<textarea value={feedback} onChange={(event) => setFeedback(event.target.value)} placeholder="填写你确认的事实、口径、原因或行动进展。提交后会保存到任务 validation 目录。" /></label><div className="detail-actions"><AppButton icon={<UploadSimple size={16} />} onClick={async () => { refresh(await api.pickFiles(task.id, "validation")); notify("附件已加入验证区"); }}>上传附件</AppButton><AppButton tone="primary" disabled={!feedback.trim()} onClick={submit}>提交补充</AppButton></div></section><ReceiptPane task={task} onReveal={() => api.revealPath(`${task.path}/outputs`)} onRefresh={async () => refresh(await api.getTask(task.id))} /></div>;
}
...
function DeliveryStage({ task, refresh, notify }: { task: TaskDetail; refresh: (task: TaskDetail) => void; notify: (m: string) => void }) {
  ...
  return <div className="delivery-layout"><section className="work-section"><header><div><h2>正式输出</h2><span>{task.outputs.length} 个文件</span></div><div className="inline-actions"><AppButton icon={<ArrowClockwise size={16} />} onClick={async () => refresh(await api.getTask(task.id))}>刷新</AppButton><AppButton icon={<FolderOpen size={16} />} onClick={() => api.revealPath(`${task.path}/outputs`)}>打开目录</AppButton></div></header><div className="output-list">{task.outputs.map((file) => <button key={file.path} onClick={() => api.revealPath(file.path)}><IconForFile file={file} /><span><strong>{file.name}</strong><small>{file.sizeKb} KB · {file.modifiedAt}</small></span><CaretRight size={15} /></button>)}{!task.outputs.length ? <div className="output-empty">目录中暂无正式输出。点击刷新可重新读取磁盘。</div> : null}</div></section><section className="work-section"><header><h2>交付工具</h2></header><button className="command-row" onClick={() => notify("Word报告将基于最新Markdown输出生成")}><FileDoc size={21} /><span><strong>生成 Word 报告</strong><small>适合正式流转与归档</small></span><CaretRight size={15} /></button><button className="command-row" onClick={() => generate("html")}><SquaresFour size={21} /><span><strong>生成 HTML 报告调度单</strong><small>适合交互阅读和分享</small></span><CaretRight size={15} /></button><button className="command-row" onClick={() => generate("skill")}><Robot size={21} /><span><strong>沉淀领域 Skill</strong><small>把可复用方法带到下一次任务</small></span><CaretRight size={15} /></button></section></div>;
}
...
function SemanticCenter({ snapshot, onBack, onRefresh, notify }: { snapshot: AppSnapshot; onBack: () => void; onRefresh: () => Promise<void>; notify: (m: string) => void }) {
  ...
  const copyMaintenancePrompt = async () => { const prompt = await api.generateSemanticPrompt(); await navigator.clipboard.writeText(prompt); notify("语义维护调度单已复制，AI只会写入待确认建议"); };
  return <main className="global-content">
    <header className="global-content-head"><div><h1>权威语义中心</h1><p>正式定义由人工确认；AI负责发现、举证和提出候选。</p></div><div className="inline-actions"><AppButton icon={<Robot size={16} />} onClick={copyMaintenancePrompt}>AI维护建议</AppButton><AppButton icon={<ArrowClockwise size={16} />} onClick={onRefresh}>刷新</AppButton><AppButton onClick={onBack}>返回任务</AppButton></div></header>
...
export function App() {
  ...
  if (view === "semantic") content = <SemanticCenter snapshot={snapshot} onBack={() => setView("tasks")} onRefresh={async () => { const next = await api.getSnapshot(); setSnapshot(next); if (next.selectedTask?.id === activeTask.id) setTask(next.selectedTask); notify("语义候选已刷新"); }} notify={notify} />;
